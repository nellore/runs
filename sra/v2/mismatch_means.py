...
incomplete.sh), coverage bigWigs from Rail-RNA run on 50K samples, AUCs
(generated by AUC.sh) for normalizing each sample to reflect coverage 
representing 40 million single-end 100-bp reads, and wiggletools v1.1.
"""
import argparse
import glob
...
if __name__ == '__main__':
    ...
    parser.add_argument(
            '--auc', type=str, required=True,
            help='path to auc.tsv generated by AUC.sh'
        )
    parser.add_argument(
    ...
    with open(args.auc) as auc_stream:
        aucs = { tokens[0] : int(tokens[1]) for tokens in
                    [line.strip().split('\t') for line in auc_stream] }
    ...
    for manifest_file in manifest_files:
        with open(manifest_file) as manifest_stream:
            srrs = filter(
                    lambda x: x not in incomplete_srrs,
                    [tokens[0].partition(':')[2] for tokens in
    ...
    for base in ['.{}' for base in 'ACGTN'] + ['']:
        'sum', ' '.join(
            [('{weight} '
              '{batch_dir}/batch_{batch_number}/coverage_bigwigs/'
              '*RR*{}.bw'.format(base)).format(
                    weight, args.batch, batch_number
                )]
