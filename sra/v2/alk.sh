...
# the ALK gene is on "Chromosome 2: 29,192,774-29,921,611 reverse strand."
# (Note we use 1-based coordinates while 0-based coordinates are returned by the mysql command
JUNC=$1
# Select junctions only in ALK
gzip -cd $JUNC | grep -w chr2 | grep -w "-" | awk '$2 >= 29192774 && $2 <= 29921611 && $3 >= 29192774 && $3 <= 29921611' | gzip >alk_junctions.tsv.gz
