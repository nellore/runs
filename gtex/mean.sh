...
$BWTOOL summary $BED $BW /dev/stdout -fill=0 | cut -f1-3,8 | awk -v rcount=$READCOUNT -v rlength=$READLENGTH '{print $1 "\t" $2 "\t" $3 "\t" $4*40000000*2/(rcount*rlength)}' >$DUMP/$SRR.mean.tsv