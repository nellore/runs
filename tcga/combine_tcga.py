...
"""
combine_tcga.py
Abhi Nellore / November 7, 2016

Reformats sorted and merged "collected junctions" files from TCGA batch runs
into files with one line per junction. Three files are written:

first_pass_tcga_junctions.tsv.gz / second_pass_tcga_junctions.tsv.gz
...
consolidated_tcga_junctions.tsv.gz
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tcga-dir', required=True,
        help='path to tcga output files; this is where the batch_* '
             'subdirectories are')
    ...
    batch_numbers = []
    i = 0
    for batch_number in xrange(30):
        manifest = os.path.join(
                    containing_dir,
                    'tcga_batch_{}.manifest'.format(batch_number)
                )
    ...
    for i, batch_number in enumerate(batch_numbers):
        for j, name in enumerate(
            ...
            column_to_final_index[
                    (batch_number, j)
                ] = sample_name_to_final_index[name]
    ...
    with open(temp_file + '.sorted') as temp_stream, gzip.open(
            os.path.join(args.output_dir, 'first_pass_tcga_junctions.tsv.gz'),
            'w'
        ) as first_pass_stream, gzip.open(
            os.path.join(args.output_dir, 'second_pass_tcga_junctions.tsv.gz'),
            'w'
        ) as second_pass_stream, gzip.open(
            os.path.join(args.output_dir,
            'consolidated_tcga_junctions.tsv.gz'), 'w'
