...
shinyUI(navbarPage(
    ...
    tabPanel('Documentation',
        ...
        h5('RSE gene'),
        p('The RangedSummarizedExperiment object for the counts summarized at the gene level using the UCSC hg38 knownGene annotation as provided ', HTML('<a href="https://lcolladotor.shinyapps.io/recount/ucsc-knowngene-hg38-genes-bp-length.Rdata">here</a>. Note that the GRanges object includes bp_length as a metadata column which is the sum of the exon widths. If two exons are overlapping, the overlapped bases are only counted once.')),
        h5('RSE exon'),
        p('The RangedSummarizedExperiment object for the counts summarized at the exon level using the UCSC hg38 knownGene annotation as provided ', HTML('<a href="https://lcolladotor.shinyapps.io/recount/ucsc-knowngene-hg38-exons.Rdata">here</a>. This GRangesList object has 1 element per gene. For each gene this object contains the reduced exons such that they are non-overlapping within a gene.')),
        h5('counts gene'),
    ...
    tabPanel('How to download with recount',
        p('The following R code shows how to use the', HTML('<a href="http://bioconductor.org/packages/recount">recount</a>'), 'Bioconductor package for downloading data ', HTML('(for now only available via <a href="https://github.com/leekgroup/recount">GitHub</a>)'), '. In this example we will download the data for study SRP009615.'),
