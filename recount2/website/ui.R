...
shinyUI(fluidPage(
    ...
    h5('RSE gene'),
    p('The RangedSummarizedExperiment object for the counts summarized at the gene level using the UCSC hg38 knownGene annotation as provided ', HTML('<a href="ucsc-knowngene-hg38-genes-bp-length.Rdata">here</a>. Note that the GRanges object includes bp_length as a metadata column which is the sum of the exon widths. If two exons are overlapping, the overlapped bases are only counted once.')),
    h5('RSE exon'),
    p('The RangedSummarizedExperiment object for the counts summarized at the exon level using the UCSC hg38 knownGene annotation as provided ', HTML('<a href="ucsc-knowngene-hg38-exons.Rdata">here</a>. This GRangesList object has 1 element per gene. For each gene this object contains the reduced exons such that they are non-overlapping within a gene.')),
    h5('counts gene'),
    ...
    tags$hr(),
    h3('The Datasets'),
    fluidRow(column(12, DT::dataTableOutput('metadata')) ),
    tags$hr(),
    h5('Authors'),
    tags$ul(
    ...
    HTML('Collado-Torres L, Nellore A, Langmead B, Leek JT. <a href="">ReCount2</a>. <i>Journal COMPLETE ME</i> <b>Volume</b> pages.'),
    tags$hr(),
    ...
    tags$hr(),
    p('This research was supported in part by COMPLETE ME.')
))